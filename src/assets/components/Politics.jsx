    ...
    async function fetchPoliticsNews() {
      try {
        const res = await fetch(
          `https://newsapi.org/v2/top-headlines?category=politics&country=us&apiKey=70b525d261a74a90a2a95c8fbe5c6eb2`
        );
                  ...
                  {articles.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2  lg:grid-cols-3 gap-6 ">
                      {articles.map((article, index) => (
                        <div
                          key={index}
                          className="bg-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition dark:bg-gray-800 duration-300"
                        >
                          {article.urlToImage && (
                            <img
